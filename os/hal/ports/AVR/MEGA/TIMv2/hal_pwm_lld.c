...
#if defined(OCR1C)
  {&TCCR1A, &TCCR1B, &OCR1AH, &OCR1AL, &OCR1BH,
  &OCR1BL, &OCR1CH, &OCR1CL, &TIFR1, &TIMSK1},
#else
  {&TCCR1A, &TCCR1B, &OCR1AH, &OCR1AL, &OCR1BH,
  &OCR1BL, NULL, NULL, &TIFR1, &TIMSK1},
#endif
#endif
#if AVR_PWM_USE_TIM2 || defined(__DOXYGEN__)
  {&TCCR2A, &TCCR2B, &OCR2A, &OCR2A, &OCR2B,
  &OCR2B, NULL, NULL, &TIFR2, &TIMSK2},
#endif
#if AVR_PWM_USE_TIM3 || defined(__DOXYGEN__)
  {&TCCR3A, &TCCR3B, &OCR3AH, &OCR3AL, &OCR3BH,
  &OCR3BL, &OCR3CH, &OCR3CL, &TIFR3, &TIMSK3},
#endif
#if AVR_PWM_USE_TIM4 || defined(__DOXYGEN__)
  {&TCCR4A, &TCCR4B, &OCR4AH, &OCR4AL, &OCR4CH,
  &OCR4CL, &OCR4CH, &OCR4CL, &TIFR4, &TIMSK4},
#endif
#if AVR_PWM_USE_TIM5 || defined(__DOXYGEN__)
  {&TCCR5A, &TCCR5B, &OCR5AH, &OCR5AL, &OCR5BH,
  &OCR5BL, &OCR5CH, &OCR5CL, &TIFR5, &TIMSK5},
#endif
};
...
void pwm_lld_init(void) {
  ...
  /* @brief This is the Fast PWM mode. */
  //TCCR3A = (1 << WGM31) | (1 << WGM30);
  //TCCR3B = (0 << WGM33) | (1 << WGM32);

  // TODO: add to the PWM structure a mode parameter to switch mode easylly.
  /* TODO: support other mode options */
  /* @brief this is the CTC mode needed for IP robot. */
...
/*
    // [CS12..CS10] = 000 ==> No source.
    *regs_table[i].tccrb &= ~(1 << CS10);
    *regs_table[i].tccrb &= ~(1 << CS11);
    *regs_table[i].tccrb &= ~(1 << CS12);
*/
/*
    // [CS12..CS10] = 001 ==> Clock is CLK/1, No prescaling.
    *regs_table[i].tccrb |=  (1 << CS10);
    *regs_table[i].tccrb &= ~(1 << CS11);
    *regs_table[i].tccrb &= ~(1 << CS12);
*/
/*
    // [CS12..CS10] = 010 ==> Clock is CLK/8
    *regs_table[i].tccrb &= ~(1 << CS10);
    *regs_table[i].tccrb |=  (1 << CS11);
    *regs_table[i].tccrb &= ~(1 << CS12);
*/

    // [CS12..CS10] = 011 ==> Clock is CLK/64
    // Prescaler used by IP-robot to be asserv.
    *regs_table[i].tccrb |=  (1 << CS10);
    *regs_table[i].tccrb |=  (1 << CS11);
    *regs_table[i].tccrb &= ~(1 << CS12);
